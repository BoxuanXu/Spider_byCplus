...
int CEncapMysql::Connect(const char* szDbIp, const char* szUser,
    ...
    SaveParam(szDbIp, szUser, szPassword, szDatabase);
    //先判断是否已经连接了, 防止重复连接
    if (IsConnected())
        return 0;
    //连接数据库
    if (mysql_real_connect(&m_connection, szDbIp, szUser, szPassword, szDatabase, 0,
            NULL, 0) == NULL) {
...
int CEncapMysql::ReConnect() {
    ...
    if (mysql_real_connect(&m_connection, m_sDbIp.c_str(), m_sUser.c_str(),
            m_sPassword.c_str(), m_sDatabase.c_str(), 0, NULL, 0) == NULL) {
        ERRMSG2("%s", mysql_error(&m_connection));
        return -1;
    }
    //设置连接标志为 true
